document.addEventListener("DOMContentLoaded", async () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (metodoPago === "Tarjeta") {
      const numTarjetaRaw = document.getElementById("numTarjeta")?.value || "";
      const fechaVenc = document.getElementById("fechaVencimiento")?.value || "";
      const cvv = document.getElementById("cvv")?.value || "";

      const numDigits = numTarjetaRaw.replace(/\D/g, "");
      if (numDigits.length !== 16) {
        showToast("Número de tarjeta inválido (debe tener 16 dígitos)", "error");
        return;
      }

      if (!/^\d{2}\/\d{2}$/.test(fechaVenc)) {
        showToast("Fecha de vencimiento inválida. Formato MM/YY", "error");
        return;
      }

      const [mmStr, yyStr] = fechaVenc.split("/");
      const mm = parseInt(mmStr, 10);
      const yy = parseInt(yyStr, 10);
      if (mm < 1 || mm > 12) {
        showToast("Mes inválido en la fecha de vencimiento", "error");
        return;
      }
    ...
    try {
      ...
      setTimeout(() => {
        window.location.href = "usuario.html?tab=compras";
      }, 3000);
    } catch (err) {
      console.error(err);
